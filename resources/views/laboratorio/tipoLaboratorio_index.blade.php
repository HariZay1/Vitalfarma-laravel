...
<div class="container mt-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3>LISTA DE<small class="text-muted"> LABORATORIO</small></h3>
  </div>
        ...
        <div class="card-body">
          ...
          <table class="table table-striped">
            ...
            <tbody>
              ...
              @foreach($lis_laboratorio as $value)
              <tr>
                <td>{{ $con++ }}</td>
                <td>{{ $value->la_nombre }}</td>
                <td>
                  <?php if($value->la_estado=='ACTIVO'){ ?>
                ...
                <td><?php echo $value->la_fecha_reg ?></td>
                <td>
                  <div class="d-flex justify-content-around">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                      <li><a class="dropdown-item" href="#" onclick="editarLaboratorio('{{ $value->id }}')">EDITAR</a></li>

                      <li><a class="dropdown-item" href="#" onclick="estadoLaboratorio('{{ $value->id }}', '{{ $value->la_estado }}')">CAMBIAR ESTADO</a></li>
                    </ul>
                    <div> <!----Separador----->
                      <div class="vr"></div>
                  </div>
                  ...
                  <div>
                    ...
                    <button title="Eliminar" onclick="eliminarLaboratorio('<?php echo $value->id; ?>')" class="btn btn-datatable btn-icon btn-transparent-dark">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="nuevoRegistroLaboratorio" method="post">
          @csrf
          <div class="row g-3">
          
            <div class="col-md-6">
              <label for="la_nombres" class="form-label">NOMBRE PRESENTACION</label>
              <input type="text" id="la_nombres" name="la_nombre" class="form-control" required placeholder="Ingresar...">
...
<script>
$("#nuevoRegistroLaboratorio").submit(function(event) {
  event.preventDefault();

  document.getElementById('boton').disabled=true;
  
  $.ajax({
    url: '/nuevoRegistroLaboratorio',
    type: 'POST',
    data: $("form").serialize(),
    success:function(){
      window.location="";
...
function estadoLaboratorio(id,la_estado){
  ...
  alertify.confirm("<p>ESTA SEGURO QUE DESEA CAMBIAR EL ESTADO? <br><br><b>ENTER</b> y <b>ESC</b> corresponden a <b>Aceptar</b> o <b>Cancelar</b></p>", function (e) {
        if (e) {
          ...
          $.post('/estadoLaboratorio', {id,la_estado}, function() {
            window.location="";
          });
...
function editarPresentacion(id,la_nombres){

$("#myModal").modal('show')
$("#id").val(id)
$("#la_nombres").val(la_nombres)
...
function eliminarPresentacion(id) {
  alertify.confirm("¿Está seguro que desea eliminar el registro?", function(e) {
    if (e) {
      alertify.success("Has pulsado  ok");
      $.post('/eliminarLaboratorio', { id:id, _token: '{{csrf_token()}}' }, function() {
        window.location = "";
      });
    } else { alertify.error("Has pulsado '" + alertify.labels.cancel + "'");
  }
